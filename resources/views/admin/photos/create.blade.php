    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.photos.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('alt_text') ? 'is-invalid' : '' }}" type="text" name="alt_text" id="alt_text" value="{{ old('alt_text', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('gallery_type') ? 'is-invalid' : '' }}" name="gallery_type" id="gallery_type" required>
                    ...
                    @foreach(App\Models\Photo::GALLERY_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('gallery_type', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
